...
barFunction <- function(var){
  Somerville_data |> 
    filter(var == {{var}}) |> 
    ggplot(aes(x = Somerville.Satisfaction.num, fill = {var})) + geom_bar(position = "fill") + labs(title = "Satisfaction and Demographic info", x = "Satisfaction Score", y = "Count", fill = var)
  
}

map(Somerville_data, barFunction)
```
