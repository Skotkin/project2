...
```{r}
Somerville_data |> group_by(Somerville_data$Gender, Somerville_data$Race.Ethnicity) |> summarize(across(Age:Race.Ethnicity, count))

Somerville_data |> summarize(across(c(Age, Gender, Race.Ethnicity), n_distinct))
...
Histogram showing the range of Satisfaction of all participants for each variable. Across all variables, participants were the least satisfied with their quality of education. This is consistent with our previous table which showed that White participants (who are notably the majority of the respondents)....
