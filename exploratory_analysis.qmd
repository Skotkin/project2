...
map(selected, countFunction) |> kable(caption = "Survey Demographic Information, Somerville, MA, 2025")
...
Somerville_data |>
  ...
  kable(caption = "Survey Respondents Race/Ethnicity Breakdown, Somerville, MA, 2025")
```
...
```{r}
findingMean(Somerville_data) |> kable(caption = "Mean Reported Satisfaction Scores by Race/Ethnicity, Somerville, MA, 2025")
```
...
histogramFunction <- function(col, label, data){
  ...
  ggplot(aes(x = {{col}})) + 
  geom_histogram() +
  labs(x = label, y = "Count", title = "Distribution of Satisfaction Ratings Among Somerville \n Happiness Survey Participants, Somerville, MA, 2025")
}
```
